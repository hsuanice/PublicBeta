...
@changelog
  v0.2.7
    - Enhancement: TSV export now preserves multiline content in fields
      • Fields containing tabs or newlines are now quoted (like CSV)
      • Allows Description field (BWF metadata) to export with actual newlines
      • TSV output is now a quoted TSV format (RFC 4180 style)
      • Backwards compatible - fields without special chars remain unquoted
      • Excel, Google Sheets, and other tools handle quoted TSV correctly
...
function LT.build_table_text(fmt, rows, COL_ORDER, header_label_from_id, get_cell_text)
  ...
  for i, r in ipairs(rows or {}) do
    ...
    for _, cid in ipairs(export_cols) do
      ...
      -- Remove trailing newlines before quoting (BWF Description often has trailing \n)
      s = s:gsub("[\r\n]+$", "")
      -- Quote fields containing special characters (for both CSV and TSV)
      if fmt == "csv" then
        -- CSV: quote if contains comma, newline, or quote
        if s:find('[,\r\n"]') then s = '"'..s:gsub('"','""')..'"' end
      elseif fmt == "tsv" then
        -- TSV: quote if contains tab or newline (preserves multiline content)
        if s:find('[\t\r\n"]') then s = '"'..s:gsub('"','""')..'"' end
      end
      line[#line+1] = s
    end
