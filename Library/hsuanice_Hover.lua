...
@about
  ...
  v2510051623
    - Pin Track compatibility: prefer REAPER native hit-testing APIs for hover detection.
      Added `get_hover_item()` and `get_mouse_track()` (native-first, SWS fallback).
      Updated `pick_on_gap()` to use `get_mouse_track()` (no BR window check).
      Updated `build_targets_for_split/trim_extend` to use `get_hover_item()` for hover hits.
      Behavior unchanged otherwise; fixes Hover mis-detection when tracks are pinned.
...
----------------------------------------
-- Mouse hit helpers (Pin-safe: REAPER native first)
----------------------------------------
function M.get_hover_item()
  local x, y = reaper.GetMousePosition()
  local hit = reaper.GetItemFromPoint(x, y, false)
  if hit then return hit end
  if reaper.BR_GetMouseCursorContext_Item then
    reaper.BR_GetMouseCursorContext()
...
function M.get_mouse_track()
  local x, y = reaper.GetMousePosition()
  local tr = nil
  if reaper.GetTrackFromPoint then
    tr = select(1, reaper.GetTrackFromPoint(x, y))
